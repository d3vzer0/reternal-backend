...
@router.get('/listeners/{worker_name}', response_model=CeleryTask)
...
@router.get('/state/listeners/get/{job_uuid}', response_model=List[ListenersOut])
...
@router.post('/listeners/{worker_name}/{listener_type}', response_model=CeleryTask)
...
@router.get('/state/listeners/create/{job_uuid}')
...
@router.delete('/listener/{worker_name}/{listener_name}', response_model=CeleryTask)
...
@router.get('/state/listener/delete/{job_uuid}')
async def delete_listener_result(job_uuid: str, current_user: dict = Depends(decode_token)):
    ''' Disable enabled listener option by c2 framework and active listener name '''
    get_workers = AsyncResult(id=job_uuid, app=celery)
    workers_result = get_workers.get() if get_workers.state == 'SUCCESS' else None
    return workers_result['response'] 
...
@router.get('/listeneroptions/{worker_name}', response_model=CeleryTask)
...
@router.get('/state/listeneroptions/get/{job_uuid}', response_model=Dict[str, Dict])
